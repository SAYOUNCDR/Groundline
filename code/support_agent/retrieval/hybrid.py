...
from support_agent.core.schemas import Evidence
from support_agent.core.text import contains_any, tokens
from support_agent.corpus.ingest import CorpusChunk, load_corpus
...
class BM25Retriever:
    ...
    def search(
        ...
        return [self.chunks[idx].as_evidence(score=score, method="bm25") for score, idx in ranked[:top_k]]
...
class QdrantSemanticRetriever:
    ...
    def search(
        ...
        response = self.client.query_points(
            ...
            query_filter=self._filter(company),
            limit=top_k,
            with_payload=True,
            with_vectors=False,
        )
        ...
        for point in response.points:
            ...
            chunk = self.chunk_by_id.get(str(payload.get("chunk_id", ""))) or self._chunk_from_payload(payload)
    ...
    def _filter(self, company: str | None) -> models.Filter | None:
        if company and company != "None":
            return models.Filter(must=[models.FieldCondition(key="company", match=models.MatchValue(value=company))])
        return None
...
class HybridRetriever:
    def __init__(self, chunks: list[CorpusChunk], semantic: QdrantSemanticRetriever | None) -> None:
        self.chunks = chunks
    ...
    def search(
        ...
        bm25_hits = self.bm25.search(expanded_query, company=company, product_area=product_area, top_k=pool_size)
        semantic_hits: list[Evidence] = []
        if self.semantic is not None:
            try:
                semantic_hits = self.semantic.search(expanded_query, company=company, top_k=pool_size)
            except Exception:
                semantic_hits = []
